...
func main() {
	...
	// Create Logger
	logsApiLogger, err := agent.NewLogger(ctx)
	if err != nil {
		logger.Fatal(err)
	}
	...
	flushLogQueue := func(force bool) {
		for !(logQueue.Empty() && (force || strings.Contains(logsStr, string(logsapi.RuntimeDone)))) {
			logger.Debug("RuntimeDone: ", strings.Contains(logsStr, string(logsapi.RuntimeDone)))
			logger.Debug("Queue length is: ", logQueue.Len())
			...
			err = logsApiLogger.SendLog(logsStr)
			if err != nil {
				logger.Error(printPrefix, err)
				return
			}
		...
		logsStr = ""
		logger.Debug("Exiting flushLogQueue loop")
	}
	...
	for {
		...
		default:
			...
			flushLogQueue(false)
			logger.Debug("Event type is: ", res.EventType)
